...
void
mcp_stdio_server::set_server_info (const std::string &name,
...
mcp_stdio_server::register_tool (const mcp::tool &tool, tool_handler handler)
{
  ...
  // Lazily register tools/list and tools/call handlers on first tool
  // registration
  if (method_handlers_.find ("tools/list") == method_handlers_.end ()) {
    method_handlers_["tools/list"]
        = [this] (const mcp::json & /*params*/,
  ...
  if (method_handlers_.find ("tools/call") == method_handlers_.end ()) {
    method_handlers_["tools/call"]
      ...
      mcp::json tool_args = params.contains ("arguments")
                                ? params["arguments"]
                                : mcp::json::object ();
      ...
      mcp::json tool_result = { { "isError", false } };
...
mcp_stdio_server::run ()
{
  ...
  while (std::getline (std::cin, line)) {
    ...
    try {
      ...
      send_response (mcp::response::create_error (
                         nullptr, mcp::error_code::parse_error,
...
mcp_stdio_server::handle_initialize (const mcp::request &req)
{
  ...
  if (!params.contains ("protocolVersion")
    ...
    return mcp::response::create_error (
               req.id, mcp::error_code::invalid_params,
  ...
  mcp::json server_info = { { "name", name_ }, { "version", version_ } };

  mcp::json result = { { "protocolVersion", negotiated_version },
...
mcp_stdio_server::process_request (const mcp::request &req)
{
  try {
    if (req.method == "initialize") {
      ...
      return mcp::response::create_success (req.id, mcp::json::object ())
          .to_json ();
    }

    if (!initialized_) {
      return mcp::response::create_error (req.id,
                                          mcp::error_code::invalid_request,
    ...
    return mcp::response::create_error (req.id, e.code (), e.what ())
        .to_json ();
  } catch (const std::exception &e) {
