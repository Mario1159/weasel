...
const std::vector<cli_command_info> g_cli_reference = {
  ...
  { "sig",
    ...
    { "sig ls", "sig handlers", "sig connections",
  ...
  { "sig connect",
    ...
    "sig connect <signal> <system> <handler> [src] [tgt]",
    { "<signal> – Full or simple C++ signal type name.",
      "<system> – System display name or type name.",
      "<handler> – Handler name as declared.",
      "[src] – Optional source entity id.",
      "[tgt] – Optional target entity id." },
  ...
  { "prefab",
    "cli",
    "Run a prefab REPL command without entering the interactive shell.",
    "weasel-cli prefab <ls|save|load|instantiate> [args...]",
    { "<ls|save|load|instantiate> – Prefab subcommand." },
    { "weasel-cli prefab ls", "weasel-cli prefab save myprefab.prefab",
      "weasel-cli prefab instantiate player 42" } },
  { "prefab ls",
    "repl",
    "List prefab assets in the loaded project.",
    "prefab ls",
    {},
    { "prefab ls" } },
  { "prefab save",
    "repl",
    "Save the active scene as a prefab scene file.",
    "prefab save [path]",
    { "[path] – Optional override path. Default: project's scenes_path + "
      "scene name + .prefab." },
    { "prefab save", "prefab save ./my_prefab.prefab" } },
  { "prefab load",
    "repl",
    "Load a prefab scene from disk and set it as the active scene.",
    "prefab load <path>",
    { "<path> – Path to the .prefab file (or prefab name)." },
    { "prefab load rsc/scenes/enemy.prefab" } },
  { "prefab instantiate",
    "repl",
    "Instantiate a prefab into the active scene under an optional parent.",
    "prefab instantiate <name|path> [parent_id]",
    { "<name|path> – Prefab name or path.",
      "[parent_id] – Optional parent entity id." },
    { "prefab instantiate enemy", "prefab instantiate enemy 42" } },
  { "check",
    "repl",
