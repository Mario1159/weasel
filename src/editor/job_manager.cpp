...
void
job_manager::add_job (const std::string &name, std::future<void> &&future)
{
...
std::vector<job_manager::job_info>
...
job_manager::get_last_finished_info ()
{
  std::lock_guard<std::mutex> const lock (m_mutex);
  if (m_last_finished_job_name.empty ()) {
    return std::nullopt;
  }
