...
build_inspector::draw ()
{
  ...
  ImGui::SameLine (ImGui::GetContentRegionAvail ().x
                   - ImGui::CalcTextSize ("Configure Project").x
  ...
  ImGui::SameLine (ImGui::GetContentRegionAvail ().x
                   - ImGui::CalcTextSize ("Build Target").x
                   - (ImGui::GetStyle ().FramePadding.x * 2.0F));

  const bool can_build = has_project && m_project_info.has_value ()
  ...
  if (ImGui::BeginChild ("##TargetsRegion", ImVec2 (-1, m_targets_height), 1)) {
    if (m_project_info.has_value ()) {
      if (m_selected_target_idx >= (int)m_project_info->targets.size ()) {
        m_selected_target_idx = -1;
      }
      for (size_t i = 0; i < m_project_info->targets.size (); ++i) {
        const bool is_selected = (m_selected_target_idx == (int)i);
        if (ImGui::Selectable (m_project_info->targets[i].name.c_str (),
                               is_selected)) {
  ...
  float const refresh_btn_w = ImGui::CalcTextSize ("Refresh Info").x
                              + (ImGui::GetStyle ().FramePadding.x * 2.0F);
  float const run_btn_w = ImGui::CalcTextSize ("Run Test").x
                          + (ImGui::GetStyle ().FramePadding.x * 2.0F);
  ImGui::SameLine (ImGui::GetContentRegionAvail ().x - refresh_btn_w - run_btn_w
                   - ImGui::GetStyle ().ItemSpacing.x);

  const bool can_run_test
      = has_project && m_project_info.has_value () && m_selected_test_idx >= 0;
  ...
  if (ImGui::BeginChild ("##TestsRegion", ImVec2 (-1, std::max (40.0F, list_h)),
                         1)) {
    if (m_project_info.has_value ()) {
      if (m_selected_test_idx >= (int)m_project_info->tests.size ()) {
        m_selected_test_idx = -1;
      }
      for (size_t i = 0; i < m_project_info->tests.size (); ++i) {
        const bool is_selected = (m_selected_test_idx == (int)i);
        if (ImGui::Selectable (m_project_info->tests[i].name.c_str (),
                               is_selected)) {
