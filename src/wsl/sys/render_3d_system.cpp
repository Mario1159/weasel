...
render_3d_system::on_render_record_draw_cmd (entt::registry &registry)
{
  ...
  if (ctx.contains<comp::singl::editor_context *> ()) {
    ...
    if (editor_ctx.grid_visible) {
      renderer->draw_grid (editor_ctx.grid_camera_pos, editor_ctx.grid_fog_center,
                           editor_ctx.grid_fog_radius);
    }

    // Draw camera gizmos in the 3D pass (editor-only, not tied to physics debug)
    if ((renderer != nullptr) && ctx.contains<comp::singl::runtime_context *> ()) {
      auto &runtime_ctx = *ctx.get<comp::singl::runtime_context *> ();
      if (!runtime_ctx.is_running) {
        comp::singl::editor_context::resolved_camera rc;
        auto *scene = runtime_ctx.scene_manager.get_active ();
        bool const have_rc = editor_ctx.resolve_game_view_camera (registry, scene, rc);

        // Colors
        glm::vec4 const default_color = glm::vec4 (150.0F/255.0F, 200.0F/255.0F, 255.0F/255.0F, 1.0F);
        glm::vec4 const selected_color = glm::vec4 (255.0F/255.0F, 185.0F/255.0F, 80.0F/255.0F, 1.0F);
        ...
        for (entt::entity const entity : view) {
          ...
          const comp::world_transform &wt = view.get<comp::world_transform> (entity);

          // compute basis
          glm::vec3 const origin = glm::vec3 (wt.value[3]);
          glm::vec3 right = glm::normalize (glm::vec3 (wt.value[0]));
          if (!std::isfinite (right.x)) { right = glm::vec3 (1,0,0);
}
          glm::vec3 up_guess = glm::normalize (glm::vec3 (wt.value[1]));
          if (!std::isfinite (up_guess.x)) { up_guess = glm::vec3 (0,1,0);
}
          glm::vec3 forward = glm::normalize (-glm::cross (right, up_guess));
          if (!std::isfinite (forward.x)) { forward = glm::vec3 (0,0,-1);
}
          ...
          float const aspect_ratio = glm::clamp (camera.aspect_ratio > 0.001F ? camera.aspect_ratio : (rc.aspect_ratio > 0.0F ? rc.aspect_ratio : 1.0F), 0.1F, 10.0F);
          float const tan_half_fov = std::tan (glm::radians (vertical_fov_deg) * 0.5F);

          float const view_distance = glm::distance (renderer->camera_position (), origin);
          float const corner_length = glm::clamp (view_distance * 0.18F, 0.45F, 5.0F);
          float const shape_term = std::sqrt (1.0F + (tan_half_fov * tan_half_fov * (1.0F + (aspect_ratio * aspect_ratio))));
          ...
          float const origin_extent = glm::clamp (corner_length * 0.12F, 0.08F, 0.35F);

          glm::vec4 const col = (entity == editor_ctx.selected_entity) ? selected_color : default_color;

          auto push_line = [&lines, col] (const glm::vec3 &a, const glm::vec3 &b) {
            lines.push_back (gfx::scene_renderer::debug_vertex{ a, col });
            lines.push_back (gfx::scene_renderer::debug_vertex{ b, col });
          };

          push_line (origin - right * origin_extent, origin + right * origin_extent);
          push_line (origin - up * origin_extent, origin + up * origin_extent);
          push_line (origin - forward * origin_extent, origin + forward * origin_extent);
