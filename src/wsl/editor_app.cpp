...
{
  m_editor_ctx = std::make_unique<comp::singl::editor_context> (*m_runtime_context);
...
editor_app::init_editor_subsystems ()
{
  ...
  m_editor_ctx->imgui_renderer
      = create_imgui_renderer (m_runtime_context->window,
                               &m_runtime_context->render_ctx);
  m_editor_ctx->debug_renderer
      = create_debug_renderer (m_runtime_context->window,
                                &m_runtime_context->render_ctx);
...
editor_app::on_render ()
{
  ...
  callbacks.build_draw_data = [this] (entt::registry &reg) {
    m_ui_layer->build_draw_data (reg);
  };
  callbacks.prepare_gpu_rsc = [this] (entt::registry &) {
    m_ui_layer->prepare_gpu_resources ();
  };
  callbacks.record_ui_draw_cmd = [this] (entt::registry &) {
    m_ui_layer->record_draw_commands ();
  };
