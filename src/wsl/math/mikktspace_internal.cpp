...
int
generate_initial_vertices_index_list (std::vector<int> &tri_list_out,
                                      ...
                                      const SMikkTSpaceContext *ctx)
{
  ...
  for (int f = 0; f < num_faces; ++f) {
    ...
    if (verts == 3) {
      ...
      // quad - decide diagonal using texture coordinates (then position
      // tie-break)
      ...
      auto dist_sq_02 = (tex2[0] - tex0[0]) * (tex2[0] - tex0[0])
                        + (tex2[1] - tex0[1]) * (tex2[1] - tex0[1]);
      auto dist_sq_13 = (tex3[0] - tex1[0]) * (tex3[0] - tex1[0])
                        + (tex3[1] - tex1[1]) * (tex3[1] - tex1[1]);
      ...
      if (dist_sq_02 < dist_sq_13)
        ...
        float pd02 = (pos2[0] - pos0[0]) * (pos2[0] - pos0[0])
                     + (pos2[1] - pos0[1]) * (pos2[1] - pos0[1])
                     + (pos2[2] - pos0[2]) * (pos2[2] - pos0[2]);
        float pd13 = (pos3[0] - pos1[0]) * (pos3[0] - pos1[0])
                     + (pos3[1] - pos1[1]) * (pos3[1] - pos1[1])
                     + (pos3[2] - pos1[2]) * (pos3[2] - pos1[2]);
        diag_02 = pd02 <= pd13;
      }
...
// attribute helpers
vec3f
get_position (const SMikkTSpaceContext *ctx, int index)
{
  int f, v;
  index_to_data (f, v, index);
  float pos[3] = { 0, 0, 0 };
...
vec3f
get_normal (const SMikkTSpaceContext *ctx, int index)
{
  int f, v;
  index_to_data (f, v, index);
  float n[3] = { 0, 0, 0 };
...
vec3f
get_texcoord (const SMikkTSpaceContext *ctx, int index)
{
  int f, v;
  index_to_data (f, v, index);
  float t[2] = { 0, 0 };
...
void
generate_shared_vertices_index_list (std::vector<int> &tri_list_in_and_out,
                                     const SMikkTSpaceContext *ctx)
{
