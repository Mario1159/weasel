...
inline bool
veq (const vec3f &a, const vec3f &b) noexcept
{
  return a.x () == b.x () && a.y () == b.y () && a.z () == b.z ();
}

inline vec3f
vadd (const vec3f &a, const vec3f &b) noexcept
{
  return vec3f{ a.x () + b.x (), a.y () + b.y (), a.z () + b.z () };
}

inline vec3f
vsub (const vec3f &a, const vec3f &b) noexcept
{
  return vec3f{ a.x () - b.x (), a.y () - b.y (), a.z () - b.z () };
}

inline vec3f
vscale (float s, const vec3f &v) noexcept
{
  return vec3f{ s * v.x (), s * v.y (), s * v.z () };
}

inline float
vdot (const vec3f &a, const vec3f &b) noexcept
{
  return a.x () * b.x () + a.y () * b.y () + a.z () * b.z ();
}

inline float
length_sq (const vec3f &v) noexcept
{
  return v.x () * v.x () + v.y () * v.y () + v.z () * v.z ();
}

inline float
length (const vec3f &v) noexcept
{
  return std::sqrt (length_sq (v));
}

inline vec3f
normalize (const vec3f &v) noexcept
{
...
inline bool
vnot_zero (const vec3f &v) noexcept
{
...
inline void
index_to_data (int &face_out, int &vert_out, int index)
{
  vert_out = index & 0x3;
  face_out = index >> 2;
}
...
void generate_shared_vertices_index_list (std::vector<int> &tri_list_in_and_out,
                                          const SMikkTSpaceContext *ctx);
