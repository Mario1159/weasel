...
// NOLINTBEGIN(readability-function-cognitive-complexity,readability-identifier-naming,
//   readability-simplify-boolean-expr, readability-isolate-declaration,
//   readability-math-missing-parentheses, bugprone-branch-clone,
//   readability-redundant-declaration, readability-braces-around-statements)
...
// Local 3-float vector type used by the algorithm.
struct vec3f
{
  float x, y, z;
};
...
static bool
veq (const vec3f &v1, const vec3f &v2)
{
  return (v1.x == v2.x) && (v1.y == v2.y) && (v1.z == v2.z);
}

static vec3f
vadd (const vec3f &v1, const vec3f &v2)
{
  return vec3f{ v1.x + v2.x, v1.y + v2.y, v1.z + v2.z };
}

static vec3f
vsub (const vec3f &v1, const vec3f &v2)
{
  return vec3f{ v1.x - v2.x, v1.y - v2.y, v1.z - v2.z };
}

static vec3f
vscale (const float fS, const vec3f &v)
{
  return vec3f{ fS * v.x, fS * v.y, fS * v.z };
}

static float
LengthSquared (const vec3f &v)
{
  return v.x * v.x + v.y * v.y + v.z * v.z;
}

static float
Length (const vec3f &v)
{
...
static float
vdot (const vec3f &v1, const vec3f &v2)
{
  return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

static bool
not_zero (const float fX)
{
  return std::fabs (fX) > FLT_MIN;
}

static bool
vnot_zero (const vec3f &v)
{
...
static bool GenerateTSpaces (STSpace psTspace[], const STriInfo pTriInfos[],
                             const SGroup pGroups[], const int iNrActiveGroups,
                             const int piTriListIn[], const float fThresCos,
                             const SMikkTSpaceContext *pContext);

static int
MakeIndex (const int iFace, const int iVert)
{
...
static void
IndexToData (int *piFace, int *piVert, const int iIndexIn)
{
...
static STSpace
AvgTSpace (const STSpace *pTS0, const STSpace *pTS1)
{
  STSpace ts_res;
  ...
  if (pTS0->fMagS == pTS1->fMagS && pTS0->fMagT == pTS1->fMagT
    ...
    if (vnot_zero (ts_res.vOs)) {
      ts_res.vOs = Normalize (ts_res.vOs);
    }
    if (vnot_zero (ts_res.vOt)) {
...
static vec3f GetPosition (const SMikkTSpaceContext *pContext, const int index);
static vec3f GetNormal (const SMikkTSpaceContext *pContext, const int index);
static vec3f GetTexCoord (const SMikkTSpaceContext *pContext, const int index);
...
// forward declaration needed now that the renaming macro is gone
bool genTangSpace (const SMikkTSpaceContext *pContext,
...
bool
genTangSpace (const SMikkTSpaceContext *pContext, const float fAngularThreshold)
{
  // count nr_triangles
  ...
  for (t = 0; t < iTotTris; t++) {
    ...
    const vec3f p0 = GetPosition (pContext, i0);
    const vec3f p1 = GetPosition (pContext, i1);
    const vec3f p2 = GetPosition (pContext, i2);
  ...
  for (f = 0; f < iNrFaces; f++) {
    ...
    for (i = 0; i < verts; i++) {
      ...
      if (pContext->m_pInterface->m_setTSpace != NULL) {
        pContext->m_pInterface->m_setTSpace (pContext, tang, bitang,
                                             pTSpace->fMagS, pTSpace->fMagT,
                                             pTSpace->bOrient, f, i);
      }
      if (pContext->m_pInterface->m_setTSpaceBasic != NULL) {
        pContext->m_pInterface->m_setTSpaceBasic (
            pContext, tang, pTSpace->bOrient == true ? 1.0F : (-1.0F), f, i);
      }
...
static NOINLINE int
FindGridCell (const float fMin, const float fMax, const float fVal)
{
                                 ...
                                 const int iNrTrianglesIn)
{
  ...
  int iMaxCount = 0;
  vec3f vMin = GetPosition (pContext, 0), vMax = vMin, vDim;
  float fMin, fMax;
  for (i = 1; i < (iNrTrianglesIn * 3); i++) {
    const int index = piTriList_in_and_out[i];

    const vec3f vP = GetPosition (pContext, index);
    if (vMin.x > vP.x) {
      vMin.x = vP.x;
    } else if (vMax.x < vP.x) {
      vMax.x = vP.x;
  ...
  for (k = 0; k < g_iCells; k++) {
    ...
    int const *pTable = &piHashTable[piHashOffsets[k]];
    ...
    if (pTmpVert != NULL) {
      for (e = 0; e < iEntries; e++) {
        int const i = pTable[e];
        const vec3f vP = GetPosition (pContext, piTriList_in_and_out[i]);
...
static void
MergeVertsFast (int piTriList_in_and_out[], STmpVert pTmpVert[],
                const SMikkTSpaceContext *pContext, const int iL_in,
                const int iR_in)
{
  // make bbox
  ...
  if (fSep >= fvMax[channel] || fSep <= fvMin[channel]) {
    ...
    for (l = iL_in; l <= iR_in; l++) {
      int const i = pTmpVert[l].index;
      const int index = piTriList_in_and_out[i];
      const vec3f vP = GetPosition (pContext, index);
      const vec3f vN = GetNormal (pContext, index);
      const vec3f vT = GetTexCoord (pContext, index);
      ...
      while (l2 < l && bNotFound) {
        ...
        const vec3f vP2 = GetPosition (pContext, index2);
        const vec3f vN2 = GetNormal (pContext, index2);
        const vec3f vT2 = GetTexCoord (pContext, index2);
        i2rec = i2;
...
static void
MergeVertsSlow (int piTriList_in_and_out[], const SMikkTSpaceContext *pContext,
                const int pTable[], const int iEntries)
{
...
static int
GenerateInitialVerticesIndexList (STriInfo pTriInfos[], int piTriList_out[],
                                  const SMikkTSpaceContext *pContext,
                                  const int iNrTrianglesIn)
{
  ...
  for (f = 0; f < pContext->m_pInterface->m_getNumFaces (pContext); f++) {
    ...
    if (verts == 3) {
...

      {
        ...
        const vec3f T0 = GetTexCoord (pContext, i0);
        const vec3f T1 = GetTexCoord (pContext, i1);
        const vec3f T2 = GetTexCoord (pContext, i2);
        const vec3f T3 = GetTexCoord (pContext, i3);
        ...
        if (distSQ_02 < distSQ_13) {
          ...
          const vec3f P1 = GetPosition (pContext, i1);
          const vec3f P2 = GetPosition (pContext, i2);
          const vec3f P3 = GetPosition (pContext, i3);
...
static vec3f
GetPosition (const SMikkTSpaceContext *pContext, const int index)
{
  int iF, iI;
  vec3f res;
  float pos[3];
...
static vec3f
GetNormal (const SMikkTSpaceContext *pContext, const int index)
{
  int iF, iI;
  vec3f res;
  float norm[3];
...
static vec3f
GetTexCoord (const SMikkTSpaceContext *pContext, const int index)
{
  int iF, iI;
  vec3f res;
  float texc[2];
...
static float
CalcTexArea (const SMikkTSpaceContext *pContext, const int indices[])
{
  const vec3f t1 = GetTexCoord (pContext, indices[0]);
  const vec3f t2 = GetTexCoord (pContext, indices[1]);
  const vec3f t3 = GetTexCoord (pContext, indices[2]);
...
static void
InitTriInfo (STriInfo pTriInfos[], const int piTriListIn[],
             const SMikkTSpaceContext *pContext, const int iNrTrianglesIn)
{
  ...
  for (f = 0; f < iNrTrianglesIn; f++) {
    // initial values
    const vec3f v1 = GetPosition (pContext, piTriListIn[f * 3 + 0]);
    const vec3f v2 = GetPosition (pContext, piTriListIn[f * 3 + 1]);
    const vec3f v3 = GetPosition (pContext, piTriListIn[f * 3 + 2]);
    const vec3f t1 = GetTexCoord (pContext, piTriListIn[f * 3 + 0]);
    const vec3f t2 = GetTexCoord (pContext, piTriListIn[f * 3 + 1]);
    const vec3f t3 = GetTexCoord (pContext, piTriListIn[f * 3 + 2]);

    const float t21x = t2.x - t1.x;
    const float t21y = t2.y - t1.y;
    const float t31x = t3.x - t1.x;
    const float t31y = t3.y - t1.y;
    const vec3f d1 = vsub (v2, v1);
    const vec3f d2 = vsub (v3, v1);

    const float fSignedAreaSTx2 = t21x * t31y - t21y * t31x;
    // assert(fSignedAreaSTx2!=0);
    vec3f const vOs = vsub (vscale (t31y, d1), vscale (t21y, d2));  // eq 18
    vec3f const vOt = vadd (vscale (-t31x, d1), vscale (t21x, d2)); // eq 19
    ...
    if (not_zero (fSignedAreaSTx2)) {
      ...
      if (not_zero (pTriInfos[f].fMagS) && not_zero (pTriInfos[f].fMagT)) {
        pTriInfos[f].iFlag &= (~GROUP_WITH_ANY);
      }
    }
  }
  ...
  while (t < (iNrTrianglesIn - 1)) {
    ...
    {
      const bool bIsDeg_a
          = (pTriInfos[t].iFlag & MARK_DEGENERATE) != 0 ? true : false;
      const bool bIsDeg_b
          = (pTriInfos[t + 1].iFlag & MARK_DEGENERATE) != 0 ? true : false;

      // bad triangles should already have been removed by
      // DegenPrologue(), but just in case check bIsDeg_a and bIsDeg_a are false
      if ((bIsDeg_a || bIsDeg_b) == false) {
        const bool bOrientA
            = (pTriInfos[t].iFlag & ORIENT_PRESERVING) != 0 ? true : false;
        const bool bOrientB
            = (pTriInfos[t + 1].iFlag & ORIENT_PRESERVING) != 0 ? true : false;
...
static bool AssignRecur (const int piTriListIn[], STriInfo psTriInfos[],
                         const int iMyTriIndex, SGroup *pGroup);
static void AddTriToGroup (SGroup *pGroup, const int iTriIndex);

static int
Build4RuleGroups (STriInfo pTriInfos[], SGroup pGroups[],
                  int piGroupTrianglesBuffer[], const int piTriListIn[],
                  const int iNrTrianglesIn)
{
...
static void
AddTriToGroup (SGroup *pGroup, const int iTriIndex)
{
...
static bool
AssignRecur (const int piTriListIn[], STriInfo psTriInfos[],
             const int iMyTriIndex, SGroup *pGroup)
{
  ...
  {
    const bool bOrient
        = (pMyTriInfo->iFlag & ORIENT_PRESERVING) != 0 ? true : false;
...
static bool
GenerateTSpaces (STSpace psTspace[], const STriInfo pTriInfos[],
                 const SGroup pGroups[], const int iNrActiveGroups,
                 const int piTriListIn[], const float fThresCos,
                 const SMikkTSpaceContext *pContext)
{
  ...
  for (g = 0; g < iNrActiveGroups; g++) {
    ...
    for (i = 0; i < pGroup->iNrFaces; i++) // triangles
    {
      ...
      for (j = 0; j < pGroup->iNrFaces; j++) {
        ...
        // project
        vec3f vOs2
            = vsub (pTriInfos[t].vOs, vscale (vdot (n, pTriInfos[t].vOs), n));
        vec3f vOt2
            = vsub (pTriInfos[t].vOt, vscale (vdot (n, pTriInfos[t].vOt), n));
        if (vnot_zero (vOs2)) {
          vOs2 = Normalize (vOs2);
        }
        if (vnot_zero (vOt2)) {
        ...
        {
          const bool bAny
              = ((pTriInfos[f].iFlag | pTriInfos[t].iFlag) & GROUP_WITH_ANY)
                        != 0
                    ? true
                    : false;
      ...
      if (!bFound) {
        ...
        pSubGroupTspace[iUniqueSubGroups]
            = EvalTspace (tmp_group.pTriMembers, iMembers, piTriListIn,
                          pTriInfos, pContext, pGroup->iVertexRepresentitive);
        ++iUniqueSubGroups;
      }
...
static STSpace
EvalTspace (int face_indices[], const int iFaces, const int piTriListIn[],
            ...
            const int iVertexRepresentitive)
{
  ...
  for (face = 0; face < iFaces; face++) {
    ...
    if ((pTriInfos[f].iFlag & GROUP_WITH_ANY) == 0) {
      vec3f n, vOs, vOt, p0, p1, p2, v1, v2;
  ...
  // normalize
  if (vnot_zero (res.vOs)) {
    res.vOs = Normalize (res.vOs);
  }
  if (vnot_zero (res.vOt)) {
...
static bool
CompareSubGroups (const SSubGroup *pg1, const SSubGroup *pg2)
{
...
static void
QuickSort (int *pSortBuffer, int iLeft, int iRight, unsigned int uSeed)
{
...
static void
BuildNeighborsFast (STriInfo pTriInfos[], SEdge *pEdges,
                    const int piTriListIn[], const int iNrTrianglesIn)
{
  ...
  for (i = 0; i < iEntries; i++) {
    ...
    if (bUnassigned_A) {
      ...
      while (j < iEntries && i0 == pEdges[j].i0 && i1 == pEdges[j].i1
        ...
        bUnassigned_B
            = pTriInfos[t].FaceNeighbors[edgenum_B] == -1 ? true : false;
...
static void
BuildNeighborsSlow (STriInfo pTriInfos[], const int piTriListIn[],
                    const int iNrTrianglesIn)
{
  int f = 0, i = 0;
  for (f = 0; f < iNrTrianglesIn; f++) {
...
static void
QuickSortEdges (SEdge *pSortBuffer, int iLeft, int iRight, const int channel,
                unsigned int uSeed)
{
  unsigned int t;
...
static void
GetEdge (int *i0_out, int *i1_out, int *edgenum_out, const int indices[],
...
static void
DegenPrologue (STriInfo pTriInfos[], int piTriList_out[],
               const int iNrTrianglesIn, const int iTotTris)
{
  ...
  while (t < iNrTrianglesIn && bStillFindingGoodOnes) {
    ...
    if (bIsGood) {
      ...
      while (bJustADegenerate && iNextGoodTriangleSearchIndex < iTotTris) {
        const bool bIsGood
            = (pTriInfos[iNextGoodTriangleSearchIndex].iFlag & MARK_DEGENERATE)
                      == 0
                  ? true
                  : false;
        if (bIsGood) {
...
static void
DegenEpilogue (STSpace psTspace[], STriInfo pTriInfos[], int piTriListIn[],
               const SMikkTSpaceContext *pContext, const int iNrTrianglesIn,
               const int iTotTris)
{
  int t = 0, i = 0;
  ...
  for (t = iNrTrianglesIn; t < iTotTris; t++) {
    ...
    const bool bSkip
        = (pTriInfos[t].iFlag & QUAD_ONE_DEGEN_TRI) != 0 ? true : false;
  ...
  for (t = 0; t < iNrTrianglesIn; t++) {
    ...
    if ((pTriInfos[t].iFlag & QUAD_ONE_DEGEN_TRI) != 0) {
      ...
      int iOrgF = -1, i = 0;
      bool bNotFound;
      unsigned char const *pV = pTriInfos[t].vert_num;
      int const iFlag = (1 << pV[0]) | (1 << pV[1]) | (1 << pV[2]);
      int iMissingIndex = 0;
      if ((iFlag & 2) == 0) {
        iMissingIndex = 1;
      } else if ((iFlag & 4) == 0) {
      ...
      while (bNotFound && i < 3) {
        ...
        const vec3f vSrcP = GetPosition (pContext, MakeIndex (iOrgF, iVert));
...
// NOLINTEND(readability-function-cognitive-complexity,readability-identifier-naming,
//   readability-simplify-boolean-expr, readability-isolate-declaration,
//   readability-math-missing-parentheses, bugprone-branch-clone,
//   readability-redundant-declaration, readability-braces-around-statements)
} // namespace mikktspace
