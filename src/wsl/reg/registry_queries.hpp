...
/*!
 * \brief Centralized query interface for reasoning about the registered engine model.
 *
...
public:
  /*!
   * \brief Constructs the query interface from the core registries.
   */
  ...
  /*!
   * \brief Returns every declared system iteration whose component contract matches the entity.
   */
  std::vector<const system_iteration_descriptor *>
  get_matching_iterations (entt::registry &registry, entt::entity entity) const;

  /*!
   * \brief Returns every declared system that has at least one iteration or event handler relevant to the entity.
   */
  ...
  /*!
   * \brief Returns every signal related to the entity through source ownership, signature components, or handlers.
   */
  std::vector<const sig::signal_debug_entry *>
  get_related_signals (entt::registry &registry, entt::entity entity) const;

  /*!
   * \brief Returns the explicit signal connections where the entity is either the signal source or the handler target.
   */
  ...
  /*!
   * \brief Finds all signals owned by a specific system.
   */
  std::vector<const sig::signal_debug_entry *>
  find_signals_owned_by_system (entt::id_type system_type_id) const;

  /*!
   * \brief Finds all event handlers owned by a specific system.
   */
  std::vector<const sig::system_handler_debug_entry *>
  find_event_handlers_owned_by_system (entt::id_type system_type_id) const;

  /*!
   * \brief Finds all signals whose signature includes the queried world component.
   */
  std::vector<const sig::signal_debug_entry *>
  find_signals_using_world_component (entt::id_type component_type_id) const;

  /*!
   * \brief Finds all event handlers whose component contract includes the queried world component.
   */
  ...
  /*!
   * \brief Finds all systems that iterate over or handle events for the queried world component.
   */
  std::vector<const system_factory_registry::system_descriptor *>
  find_systems_using_world_component (entt::id_type component_type_id) const;

  /*!
   * \brief Finds all explicit connections for a specific signal type.
   */
  std::vector<const sig::signal_connection_debug_entry *>
  find_connections_for_signal (entt::id_type signal_type_id) const;

  /*!
   * \brief Finds all explicit connections involving a specific system as either owner or target.
   */
  std::vector<const sig::signal_connection_debug_entry *>
  find_connections_for_system (entt::id_type system_type_id) const;

  /*!
   * \brief Finds all explicit connections whose signal or handler involves the queried world component.
   */
  std::vector<const sig::signal_connection_debug_entry *>
