...
/*!
 * \brief Sets the delta time for the current frame (accessible from das via
 * get_delta_time()).
 */
...
/*!
 * \brief Returns the stable type ID for a world component by display name.
 * \return The type ID, or 0 if not found.
 */
uint32_t wsl_get_component_type_id (const char *display_name);

/*!
 * \brief Fills the entity buffer with entities that have the given component.
 */
void wsl_refresh_entities_with_component (uint32_t type_id);

/*!
 * \brief Reads a float field from a das component on an entity.
 *
...
/*!
 * \brief Writes a float field to a das component on an entity.
 *
 * \param entity   Entity ID.
 * \param type_id  Stable component type ID (from get_component_type_id).
 * \param offset   Byte offset of the field within the component.
